...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 已跑通外部 AEP 收集、逐合成 ZIP、常驻 AE 快速调帧、后台直接 PNG、视频框上层包装取景、真实 AE 主程序验收、Eagle 安全配对、单机插件→SQLite→bot 索引联调和 CI；共享 NAS 架构已确定，正在收尾 Report 解析、中央服务与现场验证
- 飞书消息兼容：带文字的 `post` 富文本沿用现有 `@X.bot` 门控并进入对话/包装查询；纯图片仅在私聊、有效 `@X.bot` 或群聊回复 X.bot 时回应，当前不做视觉识别
- 后续规划：完成 Collect Files Report 解析和真实 Eagle→bot 现场验证，再进入 M2 版本与模糊输入
...
- AEP 收集工具只在新目录生成精简工程和依赖素材，不覆盖原工程；每个选中合成自动生成一个展开目录和一个经过完整性校验的同名 ZIP；选中合成的直属预合成也可以一键列出并逐个独立导出，标准化 PNG、ZIP 和 manifest 完整后再进入 Eagle 待入库流程。
