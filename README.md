...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测（查询、候选、确认、直发与云盘链接）
...
- 多终端场景下，Eagle 集成插件通过官方 Plugin API 直接把本地 AE 收集结果写入共享 Eagle Library，再向中央索引服务提交幂等入库事件。
...
- SQLite 运行数据、日志、缓存和构建产物（多终端模式下数据库仅由中央索引服务访问）
...
2. 只读定位 Eagle item 对应的 PNG/ZIP 原文件，将稳定映射写入本地 SQLite；多终端部署时改由中央索引服务统一维护。
