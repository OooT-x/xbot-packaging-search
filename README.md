...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 已跑通外部 AEP 收集、逐合成 ZIP、常驻 AE 快速调帧、高质量最终 PNG、视频框上层包装取景和 Eagle 安全配对
...
- 可选常驻 AE 快速桥接使用 `saveFrameToPng` 加速交互预览；不可用时回退 `aerender`，最终入库 PNG 始终执行一次高质量后台渲染。
