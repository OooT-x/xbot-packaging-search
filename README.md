...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 已跑通外部 AEP 收集、逐合成 ZIP、常驻 AE 快速调帧、后台直接 PNG、视频框上层包装取景、真实 AE 主程序验收、Eagle 安全配对和插件→SQLite→bot 索引联调实现，正在收尾 Report 解析、真实现场验证与 CI
- 飞书消息兼容：带文字的 `post` 富文本沿用现有 `@X.bot` 门控并进入对话/包装查询；纯图片仅在私聊、有效 `@X.bot` 或群聊回复 X.bot 时回应，当前不做视觉识别
- 后续规划：完成 Collect Files Report 解析、真实 Eagle→bot 现场验证和 CI，再进入 M2 版本与模糊输入
