...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 Eagle 入库助手已完成手动导入批次的真实归位验证；AE 包装整理面板第一版已安装到 AE 2025，可读取用户选择的合成、扫描风险、生成 PNG/manifest、引导原生 Collect Files 并压缩 ZIP
- 后续规划：用真实包装合成完成 AE 收集副本到 Eagle 正式入库的端到端验证，再把插件导入结果写入 SQLite 索引并与 bot 磁盘同步联调
...
├─ ae-tool/           AE 包装整理 ScriptUI 面板、安装器与 manifest 核心（第五阶段）
