...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 Eagle 入库助手已完成手动导入批次的真实归位验证；外部 AEP 收集工具原型已能在不启动 AE 的情况下显示全部合成，并按选中合成生成独立 AEP、素材目录和 manifest
- 后续规划：用 AE 打开外部工具生成的真实收集工程，完成画面、表达式、字体和效果验收，再接入 PNG/ZIP 交付与 Eagle 正式入库
...
- AEP 收集工具只在新目录生成精简工程和依赖素材，不覆盖原工程；标准化 PNG、ZIP 和 manifest 完整后再进入 Eagle 待入库流程。
