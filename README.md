...
- 当前开发阶段：第一阶段“文字检索闭环”已通过测试群端到端实测；M5 已跑通外部 AEP 收集、逐合成 ZIP、默认第 2 秒预览 PNG、可调帧预览窗口和 Eagle 安全配对
...
- 代表帧默认取合成第 2 秒；短于 2 秒时取最后一个有效帧。用户可在预览窗口拖动时间轴、输入秒数或前后逐帧调整，确认后才将 PNG 和帧元数据写入收集 manifest。
